...
Das {\myma{ProgrammFenster}} repräsentiert das gesamte Programm auf der grafischen Benutzeroberfläche.\\
	...
	Es wird zu dem {\myma{WahlFenster}} w gewechselt. \\
	\end{description}

\item  {\myma{{VergleichsFenster}}} \\
Das {\myma{VergleichsFenster}} ist ein Teil des {\myma{ProgrammFenster}}s, wenn zwei verschiedene {\myma{Bundestagswahl}}en verglichen werden.\\
	\begin{description}
...
Das {\myma{WahlFenster}} repräsentiert eine einzelne {\myma{Bundestagswahl}}. In einem \\{\myma{ProgrammFenster}} sind mehrere {\myma{WahlFenster}} möglich, die in Form von Tabs dargestellt werden.\\
	...
	Wechselt entweder zur {\myma{Bundes-}}, {\myma{Landes-}} und {\myma{Wahlkreisansicht}}. \\
	\item {\mymo{{bundestagswahlDarstellen(btw : Bundestagswahl)}}} \\
	Verteilt an die drei Ansichten die Arbeit, das {\myma{Bundeswahl}}-Objekt btw darzustellen. \\
	\end{description} 
...
Die {\myma{Ansicht}} ist eine abstrakte Klasse. Diese kann drei Arten von Ansichten sein, {\myma{Bundes-}} {\myma{Landes-}} und {\myma{Wahlkreisansicht}}. Diese werden im Folgenden nicht näher erläutert.\\
	...
	In jeder der drei Ansichten wird die Darstellung, des {\myma{Bundestagswahl}}-Objekts auf die drei Fenster aufgeteilt. \\
	\end{description}

\item  {\myma{{TabellenFenster}}} \\
Das {\myma{TabellenFenster}} repräsentiert das im Pflichtenheft beschriebenen Tabellenfenster. Es stellt Teile der {\myma{Bundestagswahl}}-Klasse dar. \\
	\begin{description}
	\item {\mymo{{tabellenFuellen(btw : Bundestagswahl)}}} \\
	Füllt das {\myma{TabellenFenster}} mit den notwendigen Daten des {\myma{Bundeswahl}}-Objekts.
	\end{description}

\item  {\myma{{DiagrammFenster}}} \\
Das {\myma{DiagrammFenster}} stellt das im Pflichtenheft beschriebenen Diagrammfenster. Je nach {\myma{Ansicht}} wird dort die {\myma{Sitzverteilung}}, prozentuale Anzahl der {\myma{Zweit-}} oder {\myma{Erststimmen}} angezeigt. \\
	\begin{description}
	...
	Öffnet ein {\myma{BerichtsFenster}}, in dem die Sitze der Verteilung näher erläutert werden.
	\end{description}
...
Das {\myma{KartenFenster}} korrespondiert zu dem im Pflichtenheft beschriebenen Kartenfenster. Hier wird eine Liste der {\myma{Bundesländ}}er und {\myma{Wahlkreise}} und, wenn möglich, eine kartografische Darstellung {\myma{Deutschland}}s aus einem {\myma{Bundestagswahl}}-Objekt dargestellt. \\
	...
	Färbt die einzelnen Bundesländer nach den {\myma{Partei}}en, die die meisten \\{\myma{Zweitstimme}}n haben. Diese Methode wird von zeigeInformationen benutzt.
	\end{description}
...
Die {\myma{GUISteuerung}} sorgt dafür, dass die aktuellste Version der {\myma{Bundestagswahl}} in einem {\myma{WahlFenster}} visualisiert wird. Dafür bekommt sie das aktuellste \\{\myma{Bundestagswahl}}-Objekt der {\myma{Steuerung}} und verteilt die Darstellungsarbeit an die Komponenten des {\myma{WahlFensters}}.
	...
	Diese Methode startet eine komplette Aktualisierung eines {\myma{WahlFenster}}s.
	\end{description}
...
Folgende Methoden sind enthalten:
          \begin{description}
              ...
              \item {\mymo{setzeStimme(s:Stimme) : Boolean}}\\
...
Zum Schluss wird die Methode {\mymo{prüfeDaten()}} aufgerufen.
    ...
    Darf nur als Assoziationsklasse zwischen {\myma{Wahlkreis}} und ein {\myma{Kandidat}} existieren. Dies stellt sicher, dass die Erststimme einzig auf Wahlkreisebene verändert werden kann/darf. Der Grund für diese Entscheidung ist, die Komplexität bei Veränderung auf Bundes- und Landesebene zu vermeiden.
	...
	Die Assoziationsklasse zwischen {\myma{Partei}} und {\myma{Bundesland}}. Beinhaltet eine Menge an Kandidaten.
	...
	Die Informationen werden mit de {\myma{Mandatsrechner}}
	Es wird hierbei zwischen Direktmandaten, Überhangmandaten und Ausgleichsmandaten unterschieden. Die Informationen werden in {\myma{BerichtsFenster}} visualisiert.
\end{description} 
...
\begin{description}
     \item {\myma{AbstrakterWahlgenerator}} \\
     Ist eine abstrakte Oberklasse, die als Grundlage für die spezialisierten Klassen verwendet wird.     
...
Diese Methode verteilt alle Erst- und Zweitstimmen auf die Wahlkreise der Bundestagswahl. Diese Methode muss in jeder Unterklasse von Wahlgenerator implementiert werden.     
     ...
     \item {\myma{Wahlgenerator}} \\
	Bei diesem Wahlgenerator werden alle Stimmen der jeweiligen Parteien zufällig auf Wahlkreise verteilt.
     \item {\mymo{verteileStimmen()}} \\
	Zufällige Verteilung aller Stimmen der Parteien auf Wahlkreise.     
     
     \item {\myma{NegStimmgewichtWahlgenerator}} \\
     Dieser Wahlgenerator erzeugt Bundestagswahlen, die die Voraussetzungen erfüllen, welche für die Simulation des Negativen Stimmgewichts benötigt werden.
     \item {\mymo{verteileStimmen()}} \\
Die Stimmen müssen so verteilt werden, dass bei mindestens einer Partei der prozentuale Anteil ihrer relevanten Zweitstimmen größer als der prozentuale Anteil ihrer Mandate sein. Relevante Zweitstimmen sind all diejenigen Zweitstimmen, die auf Landeslisten abgegeben werden, die keine Überhangmandate erzielen.
     \item {\myma{Stimmanteile}} \\
     Diese Klasse enthält die Zuordnung zwischen Parteien und der prozentualen Erst- und Zweitstimmen.     
...
\subsection{Chronik}
\begin{figure}[!ht]
\includegraphics[scale=0.8]{Klassendiagramme/Chronik_Ausschnitt} \caption{Chronik-Komponente} 
\end{figure}
