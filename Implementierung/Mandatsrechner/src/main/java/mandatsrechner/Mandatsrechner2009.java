...
public class Mandatsrechner2009 {
	...
	protected float berechneZuteilungsdivisor(Bundestagswahl bundestagswahl) {
		...
		////Systemprintln( bundestagswahl.getDeutschland().getEinwohneranzahl()+" Vorl�ufig ZD: "+zuteilungsdivisor);
		int sitzanzahl = 0;
	...
	public Bundestagswahl berechneSainteLague(Bundestagswahl bundestagswahl) {
		...
		if (Debug.isAktiv()) {
			//Systemprintln("Zuteilungsdivisor: " + zuteilungsdivisor);
			int summe = 0;
			for (Bundesland bl : bundestagswahl.getDeutschland().getBundeslaender()) {
				...
				summe += zahl;
				//Systemprintln(bl.getName() + ": " + zahl + " Einwohner: "
					//	+ bl.getEinwohnerzahl());
			}
			//Systemprintln("Summe aller Sitze: " + summe);
		}
