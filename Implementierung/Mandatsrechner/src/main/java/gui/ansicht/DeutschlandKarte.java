...
public class DeutschlandKarte extends JPanel {
	...
	private void skaliere() {
		skalierteKarte = grossVersion.getScaledInstance(
				(int) (0.879 * this.getHeight()), this.getHeight(),
				Image.SCALE_SMOOTH);
		skaliert = true;
	}
	...
	private void importieren() {
		...
		for (int i = 0; i < 16; i++) {
			bundeslandBilder[i] = importImg(pfad
	...
	private void erstelleGrafik() {
		...
		for (int i = 0; i < 16; i++) {
			grossVersionGraphics.drawImage(bundeslandBilderGefaerbt[i],
					(int) (posX[i] * panelBreite),
					(int) (posY[i] * panelHoehe), null);
		}
	}

	/**
	 * faerbt die Bilder der Bundeslaender entsprechend der Farbe der Partei die
	 * in diesem Bundesland den höchsten Zweitstimmenanteil hat. Das gefaerbte
			...
			try {
				...
				for (int xx = 0; xx < breite; xx++) {
					for (int yy = 0; yy < hoehe; yy++) {
						Color originalColor = new Color(aktuellesBild.getRGB(
								xx, yy), true);
